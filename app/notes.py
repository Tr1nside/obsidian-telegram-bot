...
from config import ALLOWED_USER_ID, NOTES_FOLDER, PHOTOS_FOLDER, AUDIO_TEMP_FOLDER, logger, model


class ContentType(Enum):
    TEXT: auto() # type: ignore
    CAPTION: auto() # type: ignore
    TRANSCRIPT: auto() # type: ignore
    PHOTO: auto() # type: ignore
...
def _generate_filename(type: ContentType) -> str:
    ...
    match type:
        case ContentType.TEXT:
            ...
            filename = f"Note_{timestamp}_{note_id}.md"
            return filename
        case ContentType.PHOTO:
            filename = f"photo_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{_generate_id()}.jpg"
            return filename
        case ContentType.TRANSCRIPT:
            filename = f"voice_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{_generate_id()}.ogg"
            return filename
...
def _format_content(type: ContentType, content: str | None=None, file_name: str | None=None) -> str:
    """Function for formating content to be added to the note"""
    match type:
        case ContentType.TEXT | ContentType.CAPTION:
            return f"{content}\n"
        case ContentType.TRANSCRIPT:
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        formatted_text = _format_content(type=ContentType.TEXT, content=text)
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        markdown_link = _format_content(type=ContentType.PHOTO, file_name=file_name)
        append_to_note(markdown_link)

        # Проверяем, есть ли подпись к фото
        caption = update.message.caption
        if caption:
            formatted_caption = _format_content(type=ContentType.CAPTION, content=caption)
            append_to_note(formatted_caption)
            await update.message.reply_text("Фото и подпись добавлены в заметку.")
        else:
            await update.message.reply_text(
                "Фото и подпись добавлены в заметку."
            )
    except Exception as e:
...
async def handle_voice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        ogg_path = _download_voice_file(voice)
        wav_path = _convert_ogg_to_wav(ogg_path)

        result = model.transcribe(wav_path, language="ru")
        transcript_text = result["text"]

        formatted_transcript = _format_content(ContentType.TRANSCRIPT, transcript_text)
